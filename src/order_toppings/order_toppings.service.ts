...
export class OrderToppingsService {
  constructor(
    ...
    async findOne(id: number): Promise<OrderToppings | null> {
        return this.orderToppingsRepository.findOne({ where: { id }, relations: ['topping'] });

    }

    async findByOrderId(orderId: number): Promise<OrderToppings[]> {
        return this.orderToppingsRepository.find({ where: { order: { id: orderId } }, relations: ['topping'] });
    }
